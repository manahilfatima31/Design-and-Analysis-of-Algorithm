...
def algochooser(numbers, paint, label_comparison, something, TYPE_OF_DRAW, speed):
    ...
    if something == "Bubble Sort":
        label_comparison.configure(text="Comparisons:: 0")
    ...
    elif something == "Count Sort":
        label_comparison.configure(text="Comparisons:: 0")
        countsort(numbers, 1, paint)
        if TYPE == 0:
            paint(["lawn green"] * len(numbers))
        cmp = 0

    elif something == "Bucket Sort":
        label_comparison.configure(text="Comparisons: 0")
        insertionsort(numbers, paint, label_comparison, speed)
        if TYPE == 0:
            paint(["lawn green"] * len(numbers))
...
def bubblesort(number, paint, label_comparison, speed):
    ...
    for i in range(len(number) - 1):
        ...
        for j in range(len(number) - 1 - i):
            ...
            paint(colors)
            label_comparison.configure(text="Comparisons:: " + str(cmp))
        if is_sort:
            break
        time.sleep(1 / speed)
...
def preprocess(number, k, paint, speed):
    C = [0] * (k+1)
    for num in number:
        if TYPE == 0:
            colors = ["antique white" for h in number]
...
def bucketSort(number, speed, paint, label_comparison):
    arr = []
    ...
    for i in range(slot_num):
        arr[i] = insertionsort(arr[i], paint, label_comparison, speed)
